...
"Usage: bash <(curl -sL https://raw.githubusercontent.com/jomertix/server-scripts/refs/heads/master/checkers/ip_quality.sh) [-4] [-6] [-f] [-h] [-i eth0] [-l cn|en|jp|es|de|fr|ru|pt] [-x http://usr:pwd@proxyurl:p]"
"            -4                             Test IPv4"
"            -6                             Test IPv6"
...
db_maxmind() {
    ...
    maxmind=()
    local RESPONSE=$(curl -s "http://ipwho.is/$IP")
    echo "$RESPONSE" | jq . >/dev/null 2>&1 || RESPONSE=""

    local success=$(echo "$RESPONSE" | jq -r '.success // false')
    if [[ "$success" != "true" ]]; then
        echo "Failed to get response for IP $IP"
        return 1
    fi

    maxmind[ip]=$(echo "$RESPONSE" | jq -r '.ip // "N/A"')
    maxmind[city]=$(echo "$RESPONSE" | jq -r '.city // "N/A"')
    maxmind[region]=$(echo "$RESPONSE" | jq -r '.region // "N/A"')
    maxmind[regioncode]=$(echo "$RESPONSE" | jq -r '.region_code // "N/A"')
    maxmind[country]=$(echo "$RESPONSE" | jq -r '.country // "N/A"')
    maxmind[countrycode]=$(echo "$RESPONSE" | jq -r '.country_code // "N/A"')
    maxmind[continent]=$(echo "$RESPONSE" | jq -r '.continent // "N/A"')
    maxmind[continentcode]=$(echo "$RESPONSE" | jq -r '.continent_code // "N/A"')
    maxmind[lat]=$(echo "$RESPONSE" | jq -r '.latitude // "null"')
    maxmind[lon]=$(echo "$RESPONSE" | jq -r '.longitude // "null"')
    maxmind[postal]=$(echo "$RESPONSE" | jq -r '.postal // "N/A"')
    maxmind[timezone]=$(echo "$RESPONSE" | jq -r '.timezone.id // "N/A"')
    maxmind[asn]=$(echo "$RESPONSE" | jq -r '.connection.asn // "N/A"')
    maxmind[org]=$(echo "$RESPONSE" | jq -r '.connection.org // "N/A"')
    maxmind[isp]=$(echo "$RESPONSE" | jq -r '.connection.isp // "N/A"')
    maxmind[domain]=$(echo "$RESPONSE" | jq -r '.connection.domain // "N/A"')

    if [[ ${maxmind[lat]} != "null" && ${maxmind[lon]} != "null" ]]; then
        maxmind[dms]=$(generate_dms "${maxmind[lat]}" "${maxmind[lon]}")
        maxmind[map]=$(generate_googlemap_url "${maxmind[lat]}" "${maxmind[lon]}" "")
    else
        maxmind[dms]="null"
        maxmind[map]="null"
    fi
